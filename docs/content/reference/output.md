...
describes what users get. Pick a format with `-o`, or let timus choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
timus <command> -o table   # aligned columns for reading
timus <command> -o jsonl   # one JSON object per line, for piping
timus <command> -o json    # a single JSON array
timus <command> -o csv     # spreadsheet friendly
timus <command> -o tsv     # tab-separated
timus <command> -o url     # just the URL column
timus <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
timus <command> --fields id,title,url
```
...
```bash
timus <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
timus <command>            # a table, because this is a terminal
timus <command> | wc -l    # JSONL, because this is a pipe
```
